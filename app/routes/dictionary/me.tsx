...
export async function action({ request }: ActionArgs) {
  ...
  if (intent === 'password') {
    ...
    if (!result) throw new Response('Password not changed', { status: 400 })
  }
  if (intent === 'info') {
    const name = formData.get('name') as string
    const surname = formData.get('surname') as string
    const email = formData.get('email') as string
    const result = await updateUserInfo({ email, name, surname }, user.id)
    if (!result) throw new Response('User info not changed', { status: 400 })
  }
  return json({})
}
    ...
    <>
      ...
      <div className="grid h-full w-full justify-center font-inter">
        <div className="w-full max-w-md">
          <Form method="put" className="mb-3 space-y-6 font-inter">
            <label
              htmlFor="name"
